...
select titulo, duracao, truncate(duracao * 1.03, 3) from filmes;
...
select titulo, ano from filmes order by titulo asc;

/* Mostra apenas os campos titulo e ano da tabela filmes em ordem crescente de ano. */
select titulo, ano from filmes order by ano asc;

/* Mostra apenas os campos titulo e ano da tabela filmes em ordem crescente de ano
e em ordem crescente de título. */
select titulo, ano from filmes order by ano, titulo asc;

/* Mostra apenas os campos titulo e ano da tabela filmes em ordem crescente de ano
e em ordem decrescente de título. */
select titulo, ano from filmes order by ano asc, titulo desc;
...
select titulo, ano, genero from filmes 
where ano > '2018'
order by ano, titulo;
...
select titulo, ano, classificacao from filmes 
where ano >= '2015' and ano <= '2018' and classificacao = 'livre'
order by ano, titulo;
...
select titulo, ano, genero from filmes
where titulo like '%meu%' /* Ou só no inicio ou fim, depende da sua busca*/
order by ano, titulo;
...
select titulo, max(ano) as 'Mais Novo' from filmes;


/* Mostra qual o ano mais antigo de um filme. */
select titulo, min(ano) as 'Mais antigo' from filmes;

/* Mostra qual o ano mais antigo de um filme de terror. */
select titulo, min(ano) from filmes
where genero = 'terror';
...
select genero, count(*) as Generos from filmes
group by genero
order by genero;

/* Mostra os registros agrupados por país, contando os registros. */
select pais, count(*) as Países from filmes
group by pais
order by Países desc, pais asc;
...
select genero, avg(duracao) as Média_Minutos from filmes
group by genero
order by genero;
